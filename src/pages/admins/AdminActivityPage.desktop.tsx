...
import type { Admin, PersonActivity } from '@/types'
...
function SystemMessage({ entry }: { entry: PersonActivity }) {
  ...
  } else if (entry.type === 'person_converted') {
    content = <span>Конвертовано з людини: <b>{entry.newValue}</b></span>
  } else {
...
export function AdminActivityPageDesktop() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const adminId = Number(id)
  const canEdit = usePermission('admins.viewProfiles')
  ...
  useEffect(() => {
    Promise.all([adminsApi.getById(adminId), adminsApi.getActivity(adminId)])
      .then(([a, e]) => { setAdmin(a); setEntries([...e].reverse()) })
  ...
  const handleSend = async () => {
    ...
    setSending(true)
    try {
      const entry = await adminsApi.addComment(adminId, text.trim())
  ...
  const handleDelete = async (entryId: number) => {
    Modal.confirm({
      ...
      onOk: async () => {
        await adminsApi.deleteActivity(adminId, entryId)
  ...
  const fullName = admin ? [admin.name, admin.lastName].filter(Boolean).join(' ') : ''
