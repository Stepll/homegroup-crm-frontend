...
function AttendanceGrid({ personId, group, attendance, loading }: {
  personId: number
  group?: Group
  ...
  const columns = months.map(({ year, month }) => {
    ...
    const records = attendance
      .filter(r => r.personId === personId && r.meetingDate.startsWith(prefix))
  ...
  return (
    <div style={{ ...block, padding: '12px 16px' }}>
      ...
      {!group ? (
        <span style={{ color: 'var(--color-text-tertiary)', fontSize: 13 }}>Людина не прив'язана до групи</span>
      ) : (
        <>
          {/* "Сьогодні" label */}
